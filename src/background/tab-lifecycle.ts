...
export function createTabLifecycle(ready: Promise<unknown>, maxDeadTabs = 4096): TabLifecycle {
  ...
  function markDeadInternal(tabId: number): void {
    deadTabs.add(tabId);
  ...
  // deadTabs is in-memory and starts empty on every worker (re)start; it only
  // ever learns of a closure through onRemoved firing on THIS instance. A tab
  // closed earlier — before this instance existed, or in the instant before
  // its own listeners attached — leaves no trace in it, so its absence from
  // the Set is not proof of life. Identity-free calls (no documentId) have
  // nothing else vouching for the tab either: claimDocument below returns
  // true unconditionally for them, so the Set would otherwise be their ONLY
  // gate. Ask the browser directly instead of trusting that silence.
  function canVerifyLiveness(): boolean {
    return typeof chrome !== 'undefined' && typeof chrome.tabs?.get === 'function';
  }
  async function verifiedStillOpen(tabId: number): Promise<boolean> {
    try {
  ...
  return {
    ...
    async runIfLive(tabId, task, documentId) {
      ...
      // Gated to identity-free calls only: the content-script hot capture
      // path (webRequest interceptions, MEDIA_FOUND/NOW_PLAYING/FACEBOOK_THEME)
      // always supplies a documentId and must never pay this round-trip. Panel
      // writes (pin/bindings/clear) do not, and are the ones a stale Set can
      // silently wave through.
      if (documentId == null && canVerifyLiveness() && !(await verifiedStillOpen(tabId))) {
        markDeadInternal(tabId);
        throw new ClosedTabError();
      }
      const context = contextFor(tabId);
