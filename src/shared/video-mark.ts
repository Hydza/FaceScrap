...
export function createVideoMarkFactory(epoch: string): (key: object, src: string) => string {
  ...
  return (key, src) => {
    // Strip any inner '#' before capping: this mark and the story mark are
    // joined with '#' (see the module comment above), and storage bounds an
    // overlong mark by its LAST '#' — a fragment surviving from `src` would
    // move that cut point.
    if (src && !src.startsWith('blob:')) return src.replace(/#/g, '').slice(0, 200);
