// Like createAckedLatest (acked-latest.ts), this module supplies dedupe + a
// single in-flight pump only — it does not schedule retries on a failed pump.
          ...
          if (!acknowledged) {
            if (options.splitOnFailure && inFlightCount > 1) {
              ...
              if (queue[0].failures >= options.rotateAfterFailures && queue.length > 1) {
                ...
                failed.failures = 0;
                // A fresher update for this same key may already be queued
                // behind the item that just finished failing (merged in while
                // it was in flight — see the module comment above). Re-adding
                // the stale failure behind THAT would leave two entries
                // sharing one key, which nothing outside enqueueMany ever
                // merges back together, so drop it instead of requeuing it.
                const keyOf = options.key;
                const supersededByQueued =
                  keyOf != null && queue.some((entry) => Object.is(keyOf(entry.item), keyOf(failed.item)));
                if (supersededByQueued) {
                  totalWeight -= failed.weight;
                } else {
                  queue.push(failed);
                }
