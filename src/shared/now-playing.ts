...
function retryBindings(tid: number, outbox: BindingOutbox): void {
  ...
  const delay = exponentialBackoffMs(outbox.retry++, BIND_RETRY_MIN_MS, BIND_RETRY_MAX_MS);
  outbox.timer = setTimeout(() => {
    outbox.timer = undefined;
    void pumpBindings(tid);
  }, delay);
...
function keysOf(tid: number, i: MediaItem): ItemKeys {
  ...
  const cached = itemKeysCache.get(cacheKey);
  if (cached != null && cached.url === i.url && cached.audioUrl === i.audioUrl) return cached.keys;
  const keys: ItemKeys = {
    keys: fbAssetKeys(i.url),
    audioKeys: i.audioUrl != null ? fbAssetKeys(i.audioUrl) : [],
    audioMid: i.audioUrl != null ? mediaId(i.audioUrl) : null,
  };
  if (itemKeysCache.has(cacheKey)) itemKeysCache.delete(cacheKey); // refresh insertion order
  itemKeysCache.set(cacheKey, { url: i.url, audioUrl: i.audioUrl, keys });
  if (itemKeysCache.size > ITEM_KEYS_CACHE_MAX) itemKeysCache.delete(itemKeysCache.keys().next().value as string);
  return keys;
}
...
export function playingEvidence(
  ...
  const domMatchFresh = (i: MediaItem, k: ItemKeys): boolean => {
    if (matchesActiveMediaId(i, active, historicalOwners)) return true;
...
export async function selectPlaying(tid: number, items: MediaItem[]): Promise<MediaItem[]> {
  ...
  const endorse = (keys: Set<string>, domGrade = false): void => {
    ...
    if (ref.mark != null && !isProvisionalStoryMark(ref.mark)) {
      ...
      // durableStoryCardKey): written ALONGSIDE the owner-bearing form, never
      // instead of it, so a revisit through a DIFFERENT tray entry point for
      // the identical DOM-proven card still hits — the read side above looks
      // this key up, not `sp`.
      const spKey = durableStoryCardKey(ref.mark);
      ...
      if (sp != null && (durableAnchor || (domGrade && freshGroups.has(g)))) {
        ...
        if (spKey != null) remember(markBind, `${tid}:${spKey}`, g);
      }
    }
