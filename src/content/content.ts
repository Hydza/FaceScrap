...
// Fallback MAIN-world hook installer for documents the declarative entry
// cannot reach. manifest.json now also registers page-hook.js as a MAIN-world,
// document_start content script, so a freshly navigated (or prerendered)
// document gets it installed by Chrome itself, before any page script runs —
// with no race and no help needed from here (this used to be the ONLY
// installer, an external <script> inserted from here at document_start, which
// could still lose the race against Facebook's own boot scripts). The one gap
// the declarative entry cannot cover is a document that already finished
// loading before the background reached for it: chrome.scripting.executeScript
// re-injecting content.js/content-recovery.js into an ALREADY-OPEN tab (a
// first install finding a pre-existing tab, or update-recovery — see
// service-worker.ts's onInstalled handler and content-recovery.ts). This
// function exists only for that case now; the readyState check below is what
// tells the two apart.
function ensurePageHook(): void {
  ...
  try {
    ...
    s.src = url;
    // Only latch once the browser confirms page-hook.js actually loaded. Setting
    // this before the fetch even started (as before) left a document whose fetch
    // failed — e.g. an extension update invalidating the old chrome-extension://
    // URL between navigation and injection — permanently marked as hooked with
    // no fetch/XHR wrappers ever installed, and with content-recovery.ts's later
    // passes trusting that same stale flag to keep skipping reinjection too.
    // page-hook.js is also its own last line of defence against a redundant
    // load double-patching fetch/XHR regardless (see its alreadyHooked guard),
    // so a wrong decision here can no longer corrupt an already-live hook.
    s.onload = () => {
...
// Chrome injects this content script into a PRERENDERED document the same way
// as an ordinary navigation, but the worker only accepts capture from a
// document whose sender.documentLifecycle is 'active' (see service-worker.ts's
// contentDocumentIdentity) — so a genuine capture made before activation gets
// answered retryable:false exactly like a truly dead tab would. Track that
// locally so deliverMedia's kill switch below doesn't mistake "not activated
// yet" for "dead", and re-arm on 'prerenderingchange' (fires once, exactly at
// activation) instead of leaving the document capture-dead for its whole life.
let isPrerendering =
...
async function deliverMedia(items: readonly MediaItem[]): Promise<boolean> {
  ...
  try {
    ...
    // retaining a queue that can never be acknowledged. A still-prerendering
    // document is rejected the identical way (see isPrerendering above) but is
    // not dead — activation alone makes the very same queued batch acceptable,
    // so leave the retry loop to keep it instead of tearing the detector down
    // before the user has even seen the page.
    if (response?.retryable === false && !isPrerendering) teardown();
    return false;
  } catch {
    if (!alive()) teardown();
    return false;
...
// Same pending-dedupe-and-cancel job createFrameCoalescer already does for
// imageLoadPlayingDetection above — reused here instead of a second
// hand-rolled rAF-or-setTimeout latch. Also reuses playingDetectionUsesAnimation:
// whether window.requestAnimationFrame exists is a fixed capability of this
// script's whole lifetime, never toggled mid-session, so caching that check
// once at startup — instead of re-testing it on every schedule call the way
// this path did before — cannot change what actually runs.
const themeFrameDetection = createFrameCoalescer(
  () => detectFacebookTheme(),
  (callback) =>
    playingDetectionUsesAnimation
      ? window.requestAnimationFrame(callback)
...
window.addEventListener(
  ...
  (e) => {
    ...
    if (data && data.__facescrapCtl === true && data.query === true) {
      // Receiving this at all is itself proof page-hook.js's top-level code —
      // which posts it before doing anything else — has already run in this
      // document, and already patched fetch/XHR, regardless of whether that
      // instance came from manifest.json's declarative MAIN-world entry or
      // ensurePageHook()'s runtime fallback. Latching it here (forgeable, same
      // trust boundary as the query itself — worst case a hostile page skips
      // a fallback injection that was not needed anyway) is what keeps
      // content-recovery.ts's later "did a hook survive" check honest for a
      // document whose hook only ever arrived declaratively, where
      // ensurePageHook()'s own onload confirmation never runs at all.
      contentBootstrap.__facescrapHookInjected = true;
    ...
    if (data && data.__facescrap === true) {
      ...
      if (!mediaIngressBudget.tryTake(items.length, bytes, performance.now())) {
        // The budget rejects the whole already-sanitized batch, not one item —
        // count the drop so it shows up in diagnostics instead of vanishing.
        // Policy is unchanged: content-ingress-limits.ts still sizes this so a
        // legitimate two-scan burst is admitted; only sustained excess ever lands here.
...
// Delegates to the classifier shared with page-hook.ts's pageSource() and the
// service worker's surfaceOf(), so the highlight/stories/reel precedence (and
...
// MutationObserver below): it has to, since a <video> whose src only resolves
// once buffering completes, or an <img> whose naturalWidth only clears
// MIN_MEDIA_DIMENSION_PX once decoded, changes with no childList mutation of
// its own for the observer to scope a rescan to. Left unfiltered that meant
// every 1.2s pass re-relayed every still-qualifying item, even completely
// unchanged ones: AckedBatch's key dedupe (acked-batch.ts) only merges items
// still WAITING in its queue, so one already sent and acknowledged is simply
// re-added as if new. Track the last-relayed observable state per item id and
// relay again only when that state actually differs — never merely because
// the item is still present. FIFO-bounded at the same scale as the outgoing
// queue cap so an hours-long infinite-scroll session cannot grow this map
// without limit.
const domCaptureSignatures = new Map<string, string>();
...
/** Keep only the DOM-scan items whose id is new or whose observable state
 *  (url/dimensions/thumbnail) changed since the last scan that saw it. */
function dedupeDomCapture(items: MediaItem[]): MediaItem[] {
  const changed: MediaItem[] = [];
  for (const item of items) {
    const signature = domCaptureSignature(item);
    if (domCaptureSignatures.get(item.id) === signature) continue;
    if (!domCaptureSignatures.has(item.id) && domCaptureSignatures.size >= MEDIA_QUEUE_MAX_ITEMS) {
      domCaptureSignatures.delete(domCaptureSignatures.keys().next().value as string);
    }
    domCaptureSignatures.set(item.id, signature);
    changed.push(item);
  }
  return changed;
}
...
// Reuse documentToken (already guarded above against a missing
// crypto.randomUUID) instead of calling the same unguarded API a second time —
// on a runtime without it this call used to throw here, before any now-playing
// listener below had registered.
const markVideoLoad = createVideoMarkFactory(documentToken);
...
// (e.g. the reels tab navigates to /reel/<id>?s=…). Built once from the shared
// numeric-id source (media.ts) rather than a re-spelled \d{5,20}, so this can
// never drift from isNumericMediaId's own bounds.
...
// Activation is the one event that can make an ack rejected only for
// isPrerendering suddenly acceptable (see deliverMedia). Fires at most once,
// so `once` needs no bookkeeping of its own; reset the backoff and pump
// immediately rather than waiting out whatever interval a prerendering-window
// rejection last armed.
if (isPrerendering) {
  document.addEventListener(
    'prerenderingchange',
    () => {
      isPrerendering = false;
      if (mediaRetryTimer !== undefined) {
