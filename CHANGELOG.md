...
- An in-repository MP4 remuxer that combines compatible DASH video and audio
  tracks without re-encoding or an executable runtime dependency.
- An optional download control over the active video and an `Alt+Shift+D`
  shortcut for saving the current media.
- A full-panel Settings experience with search, keyboard navigation, appearance
  controls, retention limits, and bilingual UI.
- Bounded local diagnostics with a redacted JSON export from Settings →
  Advanced.
- English-first public documentation with a complete Spanish translation,
  privacy and security notices, issue forms, and automated release packaging.

### Changed

- Redesigned the side panel's colour, density, responsive layout, and settings
  hierarchy.
- Consolidated the brand mark, media identifiers, scheduling, sanitization,
  archive checksums, and QA helpers behind shared implementations.
- Added automated policy checks for repository authorship, comment quality,
  restricted references, dead code, dependency cycles, and duplication.

### Fixed

- Restored offscreen DASH handling and propagated diagnostic context through the
  supported runtime message boundary.
- Corrected large sample tables, signed composition offsets, sync-sample tables,
  unsupported sample descriptions, and shorter-track trimming in the MP4 remuxer.
- Validated resumed byte ranges and bounded memory while downloading large tracks.
- Ensured initial content diagnostics are delivered even on otherwise idle tabs.
...
- **HD (DASH) downloads with audio**: compatible video and audio tracks are
  fetched and combined without re-encoding in an offscreen document.
- DRM (Widevine) streams are detected from the manifest and excluded as
