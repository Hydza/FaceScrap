...
test('fromPrefetchReps rejects a non-finite height instead of sorting it to the top', () => {
  ...
  const reps = [{
    representations: [
      {
        base_url: 'https://video.xx.fbcdn.net/v/legit-hd.mp4',
        mime_type: 'video/mp4',
        codecs: 'avc1.640028',
        bandwidth: 2_000_000,
        height: 1080,
      ...
      {
        base_url: 'https://video.xx.fbcdn.net/v/bogus-infinite.mp4',
        mime_type: 'video/mp4',
        codecs: 'avc1.640028',
        bandwidth: 1_000_000,
        height: Infinity,
  ...
  const pairs = fromPrefetchReps(reps);

  assert.deepEqual(pairs.map((pair) => pair.videoUrl), [
    'https://video.xx.fbcdn.net/v/legit-hd.mp4',
    'https://video.xx.fbcdn.net/v/bogus-infinite.mp4',
  ]);
});
