...
import { classifyNetworkRequest, mediaId, videoGroupKey, type MediaItem } from '../src/shared/media';
...
test('quota reclaim protects a row the panel shows as playing through a pre-canonicalization alias', async () => {
  ...
  const active: MediaItem = {
    id: mediaId(activeUrl),
    url: activeUrl,
    kind: 'image',
    source: 'story',
    origin: 'graphql',
    addedAt: 1_800_000_000_001,
  };
  const filler1 = { ...video('alt1-filler1'), addedAt: 1_800_000_000_002 };
  const filler2 = { ...video('alt1-filler2'), addedAt: 1_800_000_000_003 };
  const incoming = { ...video('alt1-incoming'), addedAt: 1_800_000_000_004 };
  await addMedia(fullTab, [active, filler1, filler2]);
  await setPlaying(fullTab, { ids: ['asset:/safe_image.php'], hasVideo: false, at: 1_800_000_000_005 });
...
test('a wall-clock rollback does not wedge the Facebook theme behind a stale future epoch', async () => {
  const tabId = nextTab++;
  const now = 1_800_000_010_000;
  const future = now + 60_000;
  await chrome.storage.session.set({ [facebookThemeKey(tabId)]: { theme: 'dark', at: future } });
