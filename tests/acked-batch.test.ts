...
test('rotating a repeatedly rejected item does not duplicate a key merged in behind it', async () => {
  interface Item { id: string; version: number }
  const delivery = createAckedBatch<Item, string>({
    maxBatch: 1,
    maxPending: 10,
  ...
  for (let retry = 0; retry < 2; retry++) {
    assert.equal(await delivery.pump(async (batch) => {
      ...
      if (retry === 0) delivery.enqueue({ id: 'a', version: 2 });
      return false;
    }), false);
  }
  // After rotateAfterFailures is reached, the stale version-1 copy must not
  // sit alongside the fresher version-2 one: exactly one entry for id 'a'.
  ...
  assert.deepEqual(sent.flat().filter((item) => item.id === 'a'), [{ id: 'a', version: 2 }]);
});
